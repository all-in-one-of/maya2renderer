...
#ifndef _RM_OCC_LIGHT_NODE_H_
#define _RM_OCC_LIGHT_NODE_H_
...
class OccLightNode : public MPxNode
{
  public:
                      OccLightNode();
    virtual          ~OccLightNode();
...
//	  static MObject aAssignedObjects;
	  ...
	  //shader parameters
	  static MObject akocc;
	  static MObject asamples;
	  static MObject amaxdist;
	  static MObject adistribution;
	  static MObject a__nonspecular;

	bool    swatchInit;

private:
	OccLightNode(const OccLightNode&);
	OccLightNode& operator=(const OccLightNode&);
};
}//namespace renderman

#endif//_RM_OCC_LIGHT_NODE_H_
