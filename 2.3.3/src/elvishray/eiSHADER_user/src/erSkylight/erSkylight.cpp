...
LIGHT(erSkylight)
	...
	void init_node()
	{
		...
		{
			printf("ERROR> erSkylight, i_env_shader().str=\"%s\", it can't be found\n", i_env_shader().str);
			assert(0 && "erSkylight's i_env_shader() node is not found");
			return;
		}
...
END_LIGHT(erSkylight)

const char *erSkylight::u_result = NULL;