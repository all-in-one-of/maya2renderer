...
//			RiReadArchive( const_cast< RtToken >( pData->getRibFileFullPath().asChar() ), NULL, RI_NULL );
	...
	static void _write(liqRibClipPlaneData* pData, const structJob &currentJob__)
	{
		...
		o.a(boost::str(boost::format("Locator(ClipPlane) is not implemented yet. [%s]")%pData->getFullPathName().asChar()));
		liquidMessage2(messageError, "Locator(ClipPlane) is not implemented yet. [%s]", pData->getFullPathName().asChar());
	}
}//namespace elvishray
