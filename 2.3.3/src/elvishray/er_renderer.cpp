...
//		_S( ei_memory( int size ) );
	...
	{
		//NOTE: this function is moved to er_writeCameraData.cpp
...
// 				}
	...
	bool Renderer::canExport()
	{
		...
		MStringArray blinns;
		IfMErrorWarn(MGlobal::executeCommand("ls -type blinn", blinns));
		...
		IfMErrorWarn(MGlobal::executeCommand("ls -type transform", transforms));
		for(std::size_t i=0; i<transforms.length(); ++i)
		{
			MStringArray srcNodes;
			MVector scale;
			IfMErrorWarn(MGlobal::executeCommand("listConnections -s true -shapes false "+transforms[i]+".scale", srcNodes));
			...
			//scaleX
			IfMErrorWarn(MGlobal::executeCommand("listConnections -s true -shapes false "+transforms[i]+".scaleX", srcNodes));
			...
			//scaleY
			IfMErrorWarn(MGlobal::executeCommand("listConnections -s true -shapes false "+transforms[i]+".scaleY", srcNodes));
			if( srcNodes.length() == 0 )
			{
			...
			//scaleZ
			IfMErrorWarn(MGlobal::executeCommand("listConnections -s true -shapes false "+transforms[i]+".scaleZ", srcNodes));
			if( srcNodes.length() == 0 )
			{
