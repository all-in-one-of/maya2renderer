...
//			RiReadArchive( const_cast< RtToken >( pData->getRibFileFullPath().asChar() ), NULL, RI_NULL );
	...
	{
		CM_TRACE_FUNC("er_writePfxToonData.cpp::write("<<pData->getFullPathName().asChar()<<","<<currentJob__.name.asChar()<<",...)");
		OutputMgr &o = Renderer::getOutputMgr();

		o.a(boost::str(boost::format("PfxToon %s is not implemented yet.")%pData->getFullPathName().asChar()));
		liquidMessage2(messageError, "PfxToon type is not implemented yet.[%s]", pData->getFullPathName().asChar());
	}
}//namespace elvishray
