...
MStatus	liqPreviewShader::doIt( const MArgList& args )
{
  ...
  if( tempString.empty() )
    ...
    preview.backPlaneShader = "liquidchecker"; // tempString + "/shaders/liquidchecker";
...
int liquidOutputPreviewShader( const string& fileName, const liqPreviewShaderOptions& options )
{
	const std::string previewLog(fileName+".log");
	...
	{//set renderer
		...
		liquid::RendererMgr::getInstancePtr()->getRenderer()->openLog(getLogFileDir()+"/"+getLogFileName()+"_preview");
	}
	...
	liquid::RendererMgr::getInstancePtr()->
		getRenderer()->preview(fileName, options);
//moved to rm_preview.cpp
