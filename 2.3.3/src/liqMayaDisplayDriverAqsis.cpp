...
PtDspyError DspyFindStringInParamList(const char *string, char **result, int n, const UserParameter *p)
{
	...
	for (i = 0; i < n; i++, p++)
	{
		if ( p->vtype == 's' && p->name[0] == string[0] && strcmp(p->name, string) == 0 )
		{
			*result = *(char **)p->value;
...
PtDspyError DspyFindMatrixInParamList(const char *string, float *result, int n, const UserParameter *p)
{
	...
	for (i = 0; i < n; i++, p++)
	{
		if (p->vtype == 'f' && p->vcount == 16 && p->name[0] == string[0] && strcmp(p->name, string) == 0)
		{
...
PtDspyError DspyFindFloatInParamList(const char *string, float *result, int n, const UserParameter *p)
{
	...
	for (i = 0; i < n; i++, p++)
	{
		if ((p->vtype == 'f' || p->vtype == 'i') && p->name[0] == string[0] && strcmp(p->name, string) == 0)
		{
			if (p->vtype == 'f')
			{
...
PtDspyError DspyFindIntInParamList(const char *string, int *result, int n, const UserParameter *p)
{
  ...
  printf("DspyFindIntInParamList name = %s n = %d\n", string, n );
	for (i = 0; i < n; i++, p++)
	{
		printf("DspyFindIntInParamList p->name = %s p->ptype = %c p->nelements = %d\n", p->name, p->vtype, p->vcount );
    
    if ((p->vtype == 'i' || p->vtype == 'f') && p->name[0] == string[0] && strcmp(p->name, string) == 0)
		{
			if (p->vtype == 'i')
			{
				*result = *(int *)p->value;
...
//  printf("[d_liqmaya] pvImage = %lu\n", (unsigned long)(void *)pvImage );
	*pvImage = imgSpecs;
